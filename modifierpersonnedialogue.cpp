...
{
    setWindowTitle("Modifier une personne");
    creeInterface();
}
...
void ModifierPersonneDialogue::creeInterface()
{
    ...
    auto boutonConfirmer = new QPushButton{tr("Confirmer")};
    auto boutonAnnuler {new QPushButton{tr("Annuler")}};
    layoutBas->addWidget(boutonAnnuler, 0, Qt::AlignBottom|Qt::AlignLeft);
    layoutBas->addWidget(boutonConfirmer, 0, Qt::AlignBottom|Qt::AlignRight);
    boutonConfirmer->setDefault(true);
    connect(boutonConfirmer, &QPushButton::clicked, this, &QDialog::accept);
